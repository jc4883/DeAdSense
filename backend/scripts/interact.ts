...
async function main() {
  ...
  var op = fUSDCx.approve({amount:(5*Math.pow(10,18)).toString(), receiver: process.env.CONTRACT_ADDRESS?process.env.CONTRACT_ADDRESS:""});
  ...
  const dasContractFactory = await ethers.getContractFactory(
   ...
   var op1 = await contract.connect(signer).addFunds(5*Math.pow(10,18));
  console.log(op1);
  // console.log("finished")

  var op2 = await contract.connect(signer).impressionRollup(["0xF0a0D2E77ADBeD1A675eBD5Aa70d8BA86d698161"], [5]);
